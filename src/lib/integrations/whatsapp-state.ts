...
export const createWhatsAppOAuthState = async (organizationId: number, userId: string) => {
  const admin = getSupabaseAdminClient();
  const stateToken = randomBytes(32).toString("hex");
  const expiresAt = new Date(Date.now() + EXPIRATION_MINUTES * 60 * 1000).toISOString();

  const { error } = await admin.from("whatsapp_oauth_states").insert({
...
export const consumeWhatsAppOAuthState = async (stateToken: string) => {
  const admin = getSupabaseAdminClient();
