...
export const createMessengerOAuthState = async (organizationId: number, userId: string) => {
  const admin = getSupabaseAdminClient();
  const stateToken = randomBytes(32).toString("hex");
  const expiresAt = new Date(Date.now() + EXPIRATION_MINUTES * 60 * 1000).toISOString();

  const { error } = await admin.from("messenger_oauth_states").insert({
...
export const consumeMessengerOAuthState = async (stateToken: string) => {
  const admin = getSupabaseAdminClient();
